...
export type ShoppingListItem={id:number;list_id:number;created_by_user_id:number;name:string;normalized_name:string;quantity:number|null;unit:string|null;note:string|null;checked:boolean;checked_by_user_id:number|null;checked_at:string|null;position:number;created_at:string;updated_at:string};
export type ShoppingList={id:number;household_id:number|null;owner_user_id:number;created_by_user_id:number;name:string;status:'active'|'completed'|'archived';default_category:string;completed_amount:number|null;completed_currency:string|null;completed_at:string|null;completed_by_user_id:number|null;linked_transaction_id:number|null;linked_credit_card_purchase_id:number|null;created_at:string;updated_at:string;items:ShoppingListItem[]};
