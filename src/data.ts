...
export async function createHouseholdInvite(name:string,email:string){const{data,error}=await supabase.rpc('create_household_invite',{p_name:name.trim(),p_email:email.trim().toLowerCase()});if(error){const code=String(error.message||'');if(code.includes('invite_email_required'))throw new Error('Informe o e-mail da pessoa que vai entrar na família.');if(code.includes('cannot_invite_self'))throw new Error('Use o e-mail do novo membro, não o e-mail do titular.');throw error}return data as {token:string;expires_in_days:number};}
...
export async function validateHouseholdInvite(token:string,email:string){const{data,error}=await supabase.rpc('validate_household_invite',{p_token:token.trim(),p_email:email.trim().toLowerCase()});if(error)throw error;return data===true;}
export async function validateRegistrationPhone(phone:string,email:string,betaToken='',familyToken=''){const{data,error}=await supabase.rpc('validate_registration_phone',{p_phone:phone.trim(),p_email:email.trim().toLowerCase(),p_beta_token:betaToken.trim()||null,p_family_token:familyToken.trim()||null});if(error)throw error;return data===true;}
export async function redeemHouseholdInvite(token:string){const{data,error}=await supabase.rpc('redeem_household_invite',{p_token:token.trim()});if(error){const raw=String(error.message||'');const tagged=(code:string,message:string)=>{const e=new Error(message);(e as any).inviteCode=code;return e};if(raw.includes('household_merge_required'))throw tagged('household_merge_required','Sua conta atual já possui dados próprios. Para não perder histórico, a união com outra família precisa ser feita pelo suporte. Sua conta continua acessível normalmente.');if(raw.includes('invite_already_used'))throw tagged('invite_already_used','Este convite familiar já foi usado. Sua conta continua normal; peça ao titular um novo convite se ainda precisar entrar nessa família.');if(raw.includes('invite_email_mismatch'))throw tagged('invite_email_mismatch','Este convite foi emitido para outro e-mail. Sua conta atual continua normal; saia e entre com o e-mail que recebeu o convite.');if(raw.includes('invalid_household_invite'))throw tagged('invalid_household_invite','Este convite familiar expirou ou não é mais válido. Sua conta continua normal; peça ao titular um novo convite.');throw error}return data;}
...
export async function connectChatChannel(channel:'telegram'|'whatsapp',options:{redirect?:boolean;direct?:boolean}={}){const{data,error}=await supabase.functions.invoke('chat-connect',{body:{action:channel,direct:options.direct===true}});if(error){let code='';try{code=(await error.context?.json())?.error||''}catch{}const messages:Record<string,string>={whatsapp_phone_missing:'Seu número de WhatsApp ainda não está salvo na conta.',whatsapp_opt_in_required:'Confirme no cadastro que você quer receber mensagens do Planeja+ no WhatsApp.'};throw new Error(messages[code]||'Não consegui conectar esse canal agora.')}if(options.redirect!==false&&data?.url)window.location.assign(data.url);return data;}
