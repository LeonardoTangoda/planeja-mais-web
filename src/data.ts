...
export async function redeemHouseholdInvite(token:string){const{data,error}=await supabase.rpc('redeem_household_invite',{p_token:token.trim()});if(error){const code=String(error.message||'');if(code.includes('household_merge_required'))throw new Error('Sua conta já possui outra família com dados. Para não perder histórico, essa união precisa ser feita pelo suporte.');if(code.includes('invite_already_used'))throw new Error('Este convite familiar já foi usado. Peça ao titular um novo convite.');if(code.includes('invalid_household_invite')||code.includes('invite_email_mismatch'))throw new Error('Este convite familiar é inválido, expirou ou pertence a outro e-mail.');throw error}return data;}
