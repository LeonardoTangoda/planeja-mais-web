...
export async function listCreditCards():Promise<CreditCard[]>{
  ...
  const[pur,ov,members]=await Promise.all([
    supabase.from('credit_card_purchases').select('id,user_id,credit_card_id,amount,currency,category,description,purchased_at,statement_due_date,source').in('credit_card_id',cardIds).order('purchased_at',{ascending:false}).limit(500),
